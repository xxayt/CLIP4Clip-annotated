    ...
    def __init__(
        ...
        for index, row in self.data.iterrows():
            if row['video_id'] not in captions.keys():
                ...
                captions[row['video_id']].append(row['sentence'])  # 储存了id对应的sentence
            else:
...
class MSRVTT_TrainDataLoader(Dataset):
        ...
        if self.unfold_sentences:
            ...
            for itm in self.data['sentences']:
                if itm['video_id'] in train_video_ids:
                    self.sentences_dict[len(self.sentences_dict)] = (itm['video_id'], itm['caption'], itm['sen_id'])
    ...
    def __getitem__(self, idx):
        if self.unfold_sentences:
            video_id, caption, sen_id = self.sentences_dict[idx]
        else:
